...
export default function AdminProjectsPage() {
  ...
  const [projects, setProjects] = useState([
    { _id: "1", title: "Quilenz Platform", type: "Team", techStack: ["Next.js", "Tailwind"], image: "" },
    { _id: "2", title: "LifeLink App", type: "Individual", techStack: ["React", "Node.js"], image: "" },
  ]);
    ...
    <div className="space-y-10">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        ...
        >
          <div className="flex items-center gap-2 text-cyan-500 mb-2">
            <Briefcase size={16} />
            <span className="text-xs font-bold uppercase tracking-[0.3em]">Project Vault</span>
          </div>
          <h1 className="text-4xl font-black text-white">
            Manage <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-blue-500">Works</span>
          </h1>
        </motion.div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-1 bg-white/5 border border-white/10 rounded-2xl">
        <div className="p-4 text-center border-r border-white/5">
          <p className="text-[10px] text-gray-500 uppercase font-bold tracking-tighter">Total Projects</p>
          <p className="text-xl font-bold text-cyan-400">{projects.length}</p>
        </div>
        <div className="p-4 text-center">
          <p className="text-[10px] text-gray-500 uppercase font-bold tracking-tighter">Live Status</p>
          <p className="text-xl font-bold text-green-500 underline decoration-green-500/30 underline-offset-4">Active</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        ...
        >
          ...
          <p className="mt-4 text-xs font-bold uppercase tracking-widest text-gray-600 group-hover:text-gray-400">New Entry</p>
        </Link>
      </div>
    </div>
  );
